...
def train_one_epoch_back(model, optimizer, data_loader, device, epoch, print_freq):
    model.train()
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq,multi_scale=False):
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = 'Epoch: [{}]'.format(epoch)
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if multi_scale: # 使用多尺度训练
            ...
            if sf != 1:
                ns = [math.ceil(x * sf / gs) * gs for x in data.shape[2:]]  # new shape (stretched to 32-multiple)
                data = F.interpolate(data, size=ns, mode='bilinear', align_corners=False)
